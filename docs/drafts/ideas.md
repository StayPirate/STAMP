...
- ~~Expand the event system. Currently only TicketEvent exists, but other event types would make sense — for example, UserEvent to log modifications to a user, or events for admin operations such as creating or removing group-role mappings.~~ → Promoted to draft: `docs/drafts/audit-trail-redesign.md`
