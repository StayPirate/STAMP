...
### Non-HTTP exceptions are intentionally non-retryable

`is_retryable_condition()` classifies only HTTP/transport exceptions. Non-httpx exceptions (e.g., SQLAlchemy `OperationalError` from a transient DB connection blip) return `False` — the task fails immediately without retry.

This is a behavioral change for `run_catch_up`: under the current blacklist mechanism, `OperationalError` is retried (it is not in the non-retryable set). Under `is_retryable_condition()`, it is not.

This is accepted because:
- DB operations within these tasks are sub-second; transient `OperationalError` during that window is statistically rare
- If PostgreSQL is down for a sustained period, the Celery worker itself is likely unable to process tasks normally
- Recovery is already guaranteed: the periodic `execute()` run (within 24h) reprocesses the same CVEs; `SyncIbsRequests` provides equivalent recovery for submission requests
- Keeping `is_retryable_condition()` HTTP-only preserves module cohesion (`http_client.py`) and avoids a slippery slope (adding SQLAlchemy, then Redis, then AMQP exceptions would dilute the function's purpose)
...
**2c.** In the "Metric placement" note: **modify** the existing statement to scope "functionally irrelevant" to the success-path metrics only (`record_created`, `record_updated`). Then add a separate rule: `record_failed()` is the exclusive responsibility of the caller (`execute()` exception handler or `fetch_single_cve` orchestrator) — `fetch_single()` MUST NOT call `record_failed()` when it intends to raise an exception. This prevents double-counting. The modified note becomes:

> **Metric placement**: success-path metric helpers (`record_created`, `record_updated`) are in-memory counter increments with no database interaction. Their placement relative to `commit_and_dispatch()` is functionally irrelevant — whether recorded before commit (inside `fetch_single()`) or after commit (in the `execute()` loop) does not affect correctness.
>
> `record_failed()` is the exclusive responsibility of the caller: the `execute()` exception handler or the `fetch_single_cve` orchestrator. `fetch_single()` MUST NOT call `record_failed()` — if it raises an exception, the caller records the failure metric after rollback and status commit.
...
**4a.** In the `run_catch_up` section (around line 487): replace the current non-retryable exception set mechanism with the `is_retryable_condition()` approach. The task decorator changes from `@celery_app.task` to `@celery_app.task(bind=True, max_retries=3)` — this fixes a pre-existing inconsistency where the spec's "Retry interaction" section (line 1185) already claims `self.retry()` usage, but the code block lacks the required `bind=True`. The retry decision becomes:

```python
@celery_app.task(bind=True, max_retries=3)
def run_catch_up(self, fetcher_name, ticket_id):
    ...
...
The explicit non-retryable set (`NotImplementedError`, `CVENotInSource`, `ValueError`) remains — these are caught BEFORE the `is_retryable_condition()` check because they represent contract violations and defensive catches (not HTTP errors). `CVENotInSource` specifically is a signal class that should never propagate to `run_catch_up` — its presence here is a defensive catch for custom `catch_up()` overrides that forgot to handle it internally. The difference from the current spec is that AFTER these are handled, the remaining exceptions are classified by `is_retryable_condition()` instead of being retried unconditionally.

Retry parameters: 3 retries with exponential backoff (5s → 10s → 20s), matching `fetch_single_cve` (see `cve-fetcher-infrastructure.md`, "Retry Policy for `fetch_single`").

**4b.** Replace the statement "applies the same retry policy as `fetch_single_cve`" and the phrase "Reserved for infrastructure failure (external service completely unreachable)" with a reference to `is_retryable_condition()` as the shared classification mechanism. The new text should state: "Retry classification: `is_retryable_condition(exc)` — retries transient HTTP conditions (network errors, HTTP 5xx, timeout, HTTP 429); fails immediately for permanent conditions (HTTP 4xx except 429, parsing errors, non-httpx exceptions). See `networking.md`, Celery Retry Classification."
...
**4d.** In the "Post-exhaustion (CVE fetchers)" paragraph: generalize the wording to cover both retry exhaustion and immediate non-retryable failure. Change "after all retries are exhausted, the task fails with no `CVESource` status write" to "when the task fails (whether from retry exhaustion or immediate non-retryable failure), no `CVESource` status write occurs." The justification remains unchanged: `run_catch_up` lacks direct `cve_id` access, and the next periodic `execute()` run overwrites the status.
...
**5b. Behavioral change**: the current spec retries all HTTP errors (both "unreachable / timeout" and "4xx/5xx" rows in the error table). With `is_retryable_condition()`, HTTP 4xx (except 429) becomes non-retryable — this aligns `correlate_submission_request` with `fetch_single_cve` behavior and eliminates wasted retry attempts on permanent errors like HTTP 403.

**5c. Retry parameters**: 3 retries with exponential backoff (5s → 10s → 20s), matching `fetch_single_cve` and `run_catch_up` (see `docs/features/platform/cve-fetcher-infrastructure.md`, "Retry Policy for `fetch_single`"). After max retries, the SR remains without correlations. Recovery is limited — see Open Question 4 in `ibs-submission-tracking.md` for the known gap. The SR will eventually be reconciled when it transitions to a final state in IBS (Pipeline 2 Step 2), or if the same package is added to another ticket (Pipeline 3).
...
**6d.** Remove the Open Point about "Completeness guard and `record_failed()` call site ambiguity" (lines 527-540). The ambiguity is fully resolved by Steps 2c and 6a together: Step 6a establishes that `fetch_single()` raises `CompletenessGuardError` (does not call `record_failed()` internally), and Step 2c establishes the general principle that `record_failed()` is the caller's exclusive responsibility. The Open Point's concern about double-counting is eliminated: `fetch_single()` raises, the caller handles rollback + status commit + metric.
