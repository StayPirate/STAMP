...
providers: token format, session management, API key validation,
...
1. Revoke all API keys for the user via
   ...
   acting_user_id=acting_user_id)`. See
...
6. Set `request.state.auth_method` to `"session"` (for JWT credentials)
   ...
   creation, `AUTH_LOGOUT_NOT_APPLICABLE` for logout).
...
2. Look up the `ApiKey` record by matching `key_hash` to the computed
   digest. If no record is found, log a WARNING with the source IP and
   fail.
   ...
   **Log rate limiting**: the WARNING emission is rate-limited to prevent
   ...
   - **Aggregated log format**: when the rate limiter unblocks an
     ...
     suppression counter:
     `"API key validation failed from {ip}. {N} additional failures suppressed in the last 60s."`
...
access to the Sentinel API. The complete API key feature — data model,
name rules, status derivation, endpoints, CLI commands, anomaly
detection, retention, and security considerations — is defined in
`docs/features/identity/api-key-management.md`.

This section documents only the authentication middleware's key
validation sub-flow (see API key validation above) and the deactivation
ordering that triggers bulk revocation.
...
For API key security considerations (hashing, expiration policy, session-only
creation, debouncing), see `docs/features/identity/api-key-management.md`
(Security Considerations).

## Cross-references

- `docs/api-spec.md` — API conventions, global responses, scoped responses
- `docs/features/identity/api-key-management.md` — API key feature: data
  model, name rules, status derivation, endpoints, CLI commands, anomaly
  detection, retention, security considerations
