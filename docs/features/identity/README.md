...
ldap-integration.md          SUSE AD sync, role mapping
...
- `ldap-integration.md` provisions users and derives roles via AD group
  mappings; `rbac.md` defines what those roles grant.
- `user-service.md` is the centralized service contract consumed by
  `user-management.md`, `ldap-integration.md`, and any future entry point
  that mutates users.
